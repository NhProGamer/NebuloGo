...
import * as utils from './randomUtilities.js'
...
export function loadFolderContent() {
    api.getFolderContent(UserId, ActualPath).then(data => document.getElementById('file-list').innerHTML = renderFolderContent(data))
}
        ...
        if (item.Type === 'file') {
            return `
                <tr class="hover:bg-gray-100 context-menu-target" data-info="${item.Name}" data-type="file">
                    ...
                    <td class="px-4 py-2">${utils.formatBytes(item.Size)}</td>
                    <td class="px-4 py-2">${mime.getType(item.Name)}</td>
                    <td class="px-4 py-2">${day}/${month}/${year} ${hours}:${minutes}</td>
                </tr>`
        } else if (item.Type === 'directory') {
            return `
                <tr class="hover:bg-gray-100 context-menu-target" data-info="${item.Name}" data-type="directory" onclick="app.changeFolder('${item.Name}')">
                    <td class="px-4 py-2 flex items-center">
